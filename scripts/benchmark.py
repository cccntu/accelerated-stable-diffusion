...
def do_inference(pipe, n_samples, precision, num_inference_steps):
    ...
    context = (
        autocast if (device.type == "cuda" and precision == "half") else nullcontext
    )
    context = nullcontext  # disable autocast
    with context("cuda"):
        images = pipe(
...
def run_benchmark(n_repeats, n_samples, precision, backend, num_inference_steps):
    ...
    * precision: 'half' or 'single' (use fp16 or fp32 tensors)
    ...
    logs = {
        "memory": 0.00
        if device.type == "cpu"
        else get_inference_memory(pipe, n_samples, precision, num_inference_steps),
        "latency": get_inference_time(
            pipe, n_samples, n_repeats, precision, num_inference_steps
        ),
    }
...
if __name__ == "__main__":
    ...
    grid = {
        ...
        "precision": ("half",),  # if device.type == "cpu" else ("single", "half"),
